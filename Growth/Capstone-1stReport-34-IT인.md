...
| (4) 주요 기능의 구현 | **1. Perceiver IO의 Attention 연산을 경량화** <br> - 본 논문에서는 범용성이 뛰어난 Perceiver IO 모델을 컴퓨팅 자원이 부족한 연합학습 클라이언트에 적용하기 위한 경량화 모델인 Linceiver IO를 제안한다. <br> - Linceiver IO는 Perceiver IO의 attention 연산에 low-rank projection을 활용하는 Linformer 구조를 적용하여 계산 복잡도를 $O(N^2)$에서 $O(kN)$으로 줄일 수 있다. <br> - 설계한 Linceiver IO의 구조는 다양한 사이즈 및 모달리티 데이터에 범용적으로 사용 가능한 모델이다. <br><br> - Low-Rank Projection에서 사용하는 k값은 학습 가능한 파라미터를 추가하여, 학습 중 동적으로 업데이트되며 입력 데이터의 특성에 따라 정보의 중요도를 판단해 효율적인 학습을 가능하게 한다. <br><br> <img src="https://github.com/yeongeunshim/CapstoneDesignProject/blob/main/linceiver%20io%20%EA%B5%AC%EC%A1%B0.jpg?raw=true" alt="Linceiver IO 구조도" width="480"/> <br><br> **2. 설계한 모델에 맞는 새로운 연합학습 구조 제안** <br> - Perceiver IO 구조를 연합학습에 적용한 사례가 없어, 본 연구에서는 Linceiver IO에 최적화된 연합학습 구조와 시스템을 독자적으로 설계하였다. <br> - Linceiver IO는 Shared Latent Perceiver와 Perceiver Head로 구성된다. <br> - Shared Latent Perceiver는 모든 클라이언트가 공유하는 백본 네트워크로, 클라이언트가 학습한 가중치를 평균화하여 업데이트된다. <br> - 각 클라이언트는 독립적인 로컬 데이터셋을 기반으로, 자체적인 Embedding Layer, Perceiver Head, 학습 가능한 Query Vector를 포함하여 로컬 학습을 수행한다. <br><br> <img src="https://github.com/yeongeunshim/CapstoneDesignProject/blob/main/%EC%97%B0%ED%95%A9%ED%95%99%EC%8A%B5%EA%B5%AC%EC%A1%B0.jpg?raw=true" alt="연합학습 구조도" width="480"/> |
| (5) 기타 | *기타 사항을 기술* ||
| (5) 기타 | *기타 사항을 기술* |
<br>
